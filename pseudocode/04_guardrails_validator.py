...
from guardrails_ai.types import OnFail as OnFailAction
...
def register_validator(name: str, data_type: str):
    def decorator(cls):
        cls.__validator_name__ = name
        cls.__validator_data_type__ = data_type
        return cls
    ...
    def validate(self, text: str):
        ...
        for validator in self.validators:
            ...
            if isinstance(result, PassResult):
                if result.value_override is not None:
                    output = result.value_override
                continue

            if isinstance(result, FailResult):
                if result.fix_value is not None:
                    output = result.fix_value
                    continue
                passed = False
                break

        return ValidationOutcome(validation_passed=passed, validated_output=output)
